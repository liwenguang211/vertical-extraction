...
typedef struct ConfigSetting {
  ...
  int effective_count = 2;     /* effective voxel point size */
  int pole_count = 5;          /* effective vertical count */
  int surround_count = 4;      /* effective surround height count */
  int stand_out_count = 15;    /* sum number of pole stand out of the surroundings */
  int search_radius = 3;       /* search radius */
...
// for down sample function
struct PNG_POINT {
  float x = 0;   // x coordinate of voxel
  float y = 0;   // y coordinate of voxel
  float z = 0;   // z coordinate of voxel
  float d = 0;   // d dis of voxel
  float count = 0;   // vector number
  int vcnt = -1;   // vertical count
  int hcnt = -1;   //herizentical count
  bool isIsolate = false;  // isolate flag
  char lineType = 0;  // 0-unkown  1-line 2-corner
  bool viewed = false; // 
  bool belongGround = false;  // if grid belong to the ground
};
...
void down_sampling_voxel(pcl::PointCloud<pcl::PointXYZI> &pl_feat,
                         double voxel_size);
/*
minPoints  // effective points among one voxel
minCcnt    // min vertical cnt number
zeroVcnt   //  pole must  >zeroVcnt 
*/
void down_sampling_voxel_png(pcl::PointCloud<pcl::PointXYZI> &pl_feat,
                         double voxel_size, int minPoints, int minVcnt, int zeroVcnt, float isolateRatio, bool onlyGrid, int searchR,std::string resultPath);
